...
> #### Filter types
> Wesog search supports several types of filters. The most common are `t` for *text search*, `i` for *reverse image search* and `p:` for *person (celebrity) search.*

> #### Filter operators
> Wesog search supports several filters operators:
> - `~`: Similar. Used for AI semantic search.
> - `=`: Equal.
> - `<`: Less than. Only for numeric filters (dates, age of a person,...).
> - `>`: Greater than. Only for numeric.
> - `≥`: Greater or equal than. Only for numeric.
> - `≤`: Less or equal than. Only for numeric.
